/*
 * Eaglson Coin Wash 洗车终端系统 - 优化版 v2.0
 * 
 * 版本: v2.0 Simplified Flow - 简化流程版
 * 日期: 2025-10-30
 * 作者: Eaglson Development Team
 * 
 * 🎯 核心优化:
 * ✅ 简化流程：从8步减少到5步（37.5%优化）
 * ✅ 4LED指示灯系统：清晰的状态反馈
 * ✅ 自动化处理：刷卡后自动验证+扣费
 * ✅ 欢迎屏幕：品牌展示和广告播放
 * ✅ 四灯全亮：洗车时所有LED点亮
...
// 简化后的超时设置
#define STATE_TIMEOUT_WELCOME_MS 60000      // 60秒欢迎循环
#define STATE_TIMEOUT_SELECT_MS 20000       // 20秒选择套餐
#define STATE_TIMEOUT_CARD_SCAN_MS 15000    // 15秒等待刷卡
#define STATE_TIMEOUT_READY_MS 2000         // 2秒准备时间
#define STATE_TIMEOUT_PROCESSING_MS 120000  // 120秒洗车
#define STATE_TIMEOUT_COMPLETE_MS 8000      // 8秒完成显示
...
struct SystemLEDs {
  bool power;               // L1: 电源灯
  LEDStatus network;        // L2: 网络灯
  LEDStatus progress;       // L3: 进度灯
  LEDStatus status;         // L4: 状态灯
...
const char* TEXT_BRAND[] = {"Eaglson Coin Wash", "Lave-Auto Eaglson", "壹狗剩洗车"};
...
void setSystemLEDStatus() {
  ...
  switch (currentState) {
    ...
    case STATE_SELECT_PACKAGE:
      ledIndicator.progress = LED_ON;  // ✅ 步骤1完成
      ledIndicator.status = LED_OFF;
      break;
      
    case STATE_CARD_SCAN:
      ledIndicator.progress = LED_ON;
      ledIndicator.status = LED_BLINK_FAST;  // ✅ 等待刷卡
      break;
      
    case STATE_SYSTEM_READY:
      ledIndicator.progress = LED_ON;
      ledIndicator.status = LED_ON;  // ✅ 准备完成
      break;
    ...
    case STATE_COMPLETE:
      ledIndicator.progress = LED_BLINK_SLOW;  // ✅ 完成闪烁
      ledIndicator.status = LED_OFF;
      break;
      
    case STATE_ERROR:
      ledIndicator.progress = LED_OFF;
      ledIndicator.status = LED_BLINK_FAST;  // 故障快闪
      break;
  }
...
void displayPackageSelection() {
  ...
  display.drawStr(15, 10, TEXT_SELECT_PACKAGE[currentLanguage]);
...
void setup() {
  ...
  Serial.println("Simplified Flow Edition - 简化流程版");
  Serial.println("🎯 5步流程 + 4LED指示灯系统");
